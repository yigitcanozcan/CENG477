...
void display()
{
    ...
    renderText(topRightText, 600-20*topRightText.length(), 1000-30 , scale, glm::vec3(1, 1, 0));
...
unsigned postClearanceState[9][9];
void handlePostFallStateClearence(){
    unsigned clearanceMap = 0b111111111111;
    for(int i = 0 ; i<9; i++){
        for(int j = 0 ; j<9; j++){
            clearanceMap &= state[i][j];
            postClearanceState[i][j] = 0;
        }
    }
    int newLevelIndexer = 0;
    for(int i = 0; i < 12; i++){
        if(!getNthBit(clearanceMap,i)){
            for(int j = 0; j < 9; j++){
                for(int k = 0; k < 9; k++){
                    if(getNthBit(state[j][k],i)){
                        postClearanceState[j][k] |= 1<<newLevelIndexer;
                    }
                }
            }
            newLevelIndexer += 1;
    ...
    int totalState = 0;
    int totalClearedState = 0; 

    for(int i=0; i<9; i++){
        for(int j=0; j<9; j++){
            for(int k=0; k<12; k++){
                if(getNthBit(state[i][j],k)){
                    totalState += 1;
                }
                if(getNthBit(postClearanceState[i][j],k)){
                    totalClearedState += 1;
                }
            }
        }    
    ...
    assignment9by9(state,postClearanceState);
    int difference = totalState-totalClearedState;
    if(difference){
        points += difference;
        vectorsValidate = false;
    }
